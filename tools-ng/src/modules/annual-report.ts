...
export async function generateAnnualReport(
  ...
  const typeCommits: Record<string, number> = { FEATURE: 0, FIX: 0, REFACTOR: 0, DOCS: 0, TEST: 0, OTHER: 0 };
    ...
    try {
      ...
      for (const gitDir of gitDirs) {
        ...
        try {
          ...
          logger.warn(`    Error in ${projectName}: ${(error as Error).message}`);
        }
      }
    } catch (error) {
...
function classifyCommit(message: string): string {
  ...
  if (lower.includes('fix') || lower.includes('修复')) return 'FIX';
  if (lower.includes('refactor') || lower.includes('重构')) return 'REFACTOR';
  if (lower.includes('docs') || lower.includes('文档')) return 'DOCS';
