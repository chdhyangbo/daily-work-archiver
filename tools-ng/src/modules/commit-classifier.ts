 ...
 * 提交分类规则
 */
const ClassificationRules: Record<string, { patterns: RegExp[]; keywords: string[]; icon: string; color: string }> = {
  FEATURE: {
...
export async function analyzeCommits(
  ...
  try {
    ...
    logger.error(`Error analyzing commits: ${(error as Error).message}`);
    return {};
  }
}
 ...
 * 为提交信息提供改进建议
 */
export function suggestCommitMessage(subject: string): string | null {
