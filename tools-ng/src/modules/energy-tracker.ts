...
export async function initEnergyTracker(dataDir: string): Promise<string> {
...
export async function logEnergy(
  ...
  const data = await readJsonFile<{ logs: EnergyLog[] }>(indexFile) || { logs: [] };
  ...
  if (task) logger.info(`任务: ${task}`);

  return log;
}

export async function getDailyEnergySummary(dataDir: string, date?: string): Promise<DailyEnergySummary | null> {
  const energyDir = await initEnergyTracker(dataDir);
  const indexFile = path.join(energyDir, 'index.json');

  const data = await readJsonFile<{ logs: EnergyLog[] }>(indexFile);
  ...
  const hourEnergy: Record<string, number[]> = {};
  for (const log of dayLogs) {
    const hour = log.time.split(':')[0];
    if (!hourEnergy[hour]) hourEnergy[hour] = [];
...
export async function analyzeEnergyPattern(dataDir: string, days: number = 30): Promise<EnergyPattern> {
  const energyDir = await initEnergyTracker(dataDir);
  const indexFile = path.join(energyDir, 'index.json');
  ...
  const averageEnergyByHour: Record<string, number> = {};
  ...
  let energyTrend: EnergyPattern['energyTrend'] = 'stable';
...
export async function generateEnergyReport(dataDir: string, days: number = 7): Promise<string> {
  let report = '## 💫 能量与情绪报告\n\n';
  ...
  for (let i = 0; i < days; i++) {
    ...
    if (summary) {
      ...
      report += `- 主导情绪: ${MOOD_LABELS[summary.dominantMood as keyof typeof MOOD_LABELS]}\n`;
