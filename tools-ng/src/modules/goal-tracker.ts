...
const INFJ_REMINDER_MESSAGES = {
  upcoming: (goal: string, days: number) => 
    `「${goal}」对你很重要，还有 ${days} 天，今天可以为它做些什么？`,
  today: (goal: string) => 
    `今天是推进「${goal}」的好时机，需要我帮你拆解第一步吗？`,
  overdue: (goal: string) => 
    `「${goal}」暂时被搁置了，这很正常。什么时候重新拾起它？`,
  completed: (goal: string) => 
    `🎉 太棒了！「${goal}」完成了！这对你意味着什么？`,
  inspiration: (goal: string) => 
    `想想「${goal}」实现后的样子，那会是多么美好的改变？`
};
...
export async function initGoalTracker(dataDir: string): Promise<string> {
...
export async function addGoal(
  ...
  const data = await readJsonFile<GoalData>(indexFile) || { goals: [], lastReview: new Date().toISOString() };
...
export async function updateGoalProgress(
  ...
  goalId: string,
  progress: number,
  keyResultUpdates?: { keyResultId: string; progress: number }[]
): Promise<Goal> {
  const goalDir = path.join(dataDir, 'goals');
  const indexFile = path.join(goalDir, 'index.json');
  
  const data = await readJsonFile<GoalData>(indexFile);
  if (!data) {
...
export async function getActiveGoals(dataDir: string, level?: Goal['level']): Promise<Goal[]> {
  const goalDir = path.join(dataDir, 'goals');
  const indexFile = path.join(goalDir, 'index.json');
...
export async function getGoalsDueSoon(dataDir: string, days: number = 7): Promise<Goal[]> {
...
export async function getOverdueGoals(dataDir: string): Promise<Goal[]> {
  const goals = await getActiveGoals(dataDir);
  const now = new Date();
...
export async function generateGoalReport(dataDir: string, level?: Goal['level']): Promise<string> {
  ...
  // 按层级分组
  if (goals.length > 0) {
    const levels: Goal['level'][] = ['quarterly', 'monthly', 'weekly', 'daily'];
...
export async function reviewGoals(dataDir: string): Promise<string> {
  const goalDir = path.join(dataDir, 'goals');
  const indexFile = path.join(goalDir, 'index.json');
