...
export async function findGitDirs(rootPath: string, maxDepth: number = 3): Promise<string[]> {
  try {
    ...
    const validGitDirs = gitDirs.filter(gitDir => {
      ...
      // 排除 .git 内部的 .git
      if ((normalizedPath.match(/\.git/g) || []).length > 1) return false;
      return true;
    });
...
export async function getCommitStats(hash: string, projectPath?: string): Promise<{ insertions: number; deletions: number }> {
  try {
...
function classifyCommitType(subject: string): GitCommit['type'] {
