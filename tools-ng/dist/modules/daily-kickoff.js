...
import { getGoalsDueSoon, getOverdueGoals } from './goal-tracker.js';
...
export async function runDailyKickoff(dataDir) {
    ...
    const yesterdayData = await readJsonFile(yesterdayReport);
    ...
    const kickoffIndex = await readJsonFile(allKickoffsFile) || { kickoffs: [] };
...
function generateFocusPlan(dataDir) {
    const now = new Date();
...
function generateKickoffTemplate(today, dueSoonGoals, overdueGoals) {
...
export async function getKickoffHistory(dataDir, days = 7) {
    const kickoffDir = path.join(dataDir, 'kickoffs');
    const allKickoffsFile = path.join(kickoffDir, 'index.json');
    const data = await readJsonFile(allKickoffsFile);
...
//# sourceMappingURL=daily-kickoff.js.map