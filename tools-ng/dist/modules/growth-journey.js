...
export async function initGrowthJourney(dataDir) {
...
export async function addSkill(dataDir, name, category, level = 1) {
    const growthDir = await initGrowthJourney(dataDir);
    const growthFile = path.join(growthDir, 'data.json');
    const data = await readJsonFile(growthFile);
    if (!data) {
...
export async function updateSkillLevel(dataDir, skillId, newLevel, milestone) {
    const growthDir = await initGrowthJourney(dataDir);
    const growthFile = path.join(growthDir, 'data.json');
    const data = await readJsonFile(growthFile);
    if (!data) {
...
export async function addMilestone(dataDir, title, description, category = 'technical') {
    const growthDir = await initGrowthJourney(dataDir);
    const growthFile = path.join(growthDir, 'data.json');
    const data = await readJsonFile(growthFile);
    if (!data) {
...
export async function updateCapabilities(dataDir, capabilities) {
    const growthDir = await initGrowthJourney(dataDir);
    const growthFile = path.join(growthDir, 'data.json');
    const data = await readJsonFile(growthFile);
    if (!data) {
...
export async function generateGrowthReport(dataDir) {
    const growthDir = await initGrowthJourney(dataDir);
    const growthFile = path.join(growthDir, 'data.json');
    const data = await readJsonFile(growthFile);
    if (!data) {
...
export async function getGrowthTimeline(dataDir, months = 6) {
    const growthDir = await initGrowthJourney(dataDir);
    const growthFile = path.join(growthDir, 'data.json');
    const data = await readJsonFile(growthFile);
    if (!data) {
...
//# sourceMappingURL=growth-journey.js.map