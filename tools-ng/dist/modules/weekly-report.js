 ...
 * 获取指定日期的 Git 统计数据
 */
export async function getGitStatsByDay(date, projectPaths, author) {
    const stats = {
        commits: 0,
        insertions: 0,
        deletions: 0,
 ...
 * 生成ASCII燃尽图
 */
export function generateBurndownChart(gitStats, dates) {
    let chart = '```\n本周提交趋势:\n\n';
 ...
 * 生成项目时间分配图（ASCII）
 */
export function generateTimePieChart(timeStats, dates) {
    // 汇总一周的项目时间
    const projectTotals = {};
    let totalHours = 0;
    dates.forEach(date => {
 ...
 * 生成完整的周报
 */
export function generateWeeklyReport(weekStart, dates, gitStats, timeStats) {
 ...
 * 获取时间追踪数据（按天）
 */
export async function getTimeStatsByDay(date, dataDir) {
    ...
    try {
        ...
        logger.warn(`Error reading time tracking for ${date}: ${error.message}`);
        return {
            hours: 0,
            types: {},
            projects: {}
