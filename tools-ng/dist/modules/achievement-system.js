 ...
 * 获取Git统计数据
 */
export async function getGitStatistics(projectPaths, author) {
    const stats = {
        totalCommits: 0,
        projectCount: 0,
        maxStreak: 0,
 ...
 * 主函数：检查成就
 */
export async function runAchievementSystem(action, projectPaths, author, outputPath) {
    ...
    if (action === 'check') {
        ...
        if (data) {
            ...
            data.achievements.list.forEach((ach) => {
                if (ach.unlocked) {
