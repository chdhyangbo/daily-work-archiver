...
export async function initFocusProtector(dataDir) {
...
export async function startFocusSession(dataDir, task) {
    const focusDir = await initFocusProtector(dataDir);
    const indexFile = path.join(focusDir, 'index.json');
    const data = await readJsonFile(indexFile) || { sessions: [] };
    const now = new Date();
    const session = {
        id: `focus_${Date.now()}`,
...
export async function endFocusSession(dataDir, sessionId, quality, notes) {
    const focusDir = await initFocusProtector(dataDir);
    const indexFile = path.join(focusDir, 'index.json');
...
export async function logInterruption(dataDir, sessionId, type, duration) {
    const focusDir = await initFocusProtector(dataDir);
    const indexFile = path.join(focusDir, 'index.json');
    const data = await readJsonFile(indexFile);
    if (!data) {
...
export async function analyzeFocusPattern(dataDir, days = 30) {
    const focusDir = await initFocusProtector(dataDir);
    const indexFile = path.join(focusDir, 'index.json');
    const data = await readJsonFile(indexFile);
...
export async function generateFocusReport(dataDir, days = 7) {
    let report = '## 🎯 专注力报告\n\n';
...
//# sourceMappingURL=focus-protector.js.map