...
export async function aggregateGitActivities(config, options) {
    ...
    for (const rootPath of config.projectPaths) {
        try {
            ...
            logger.error(`Error scanning ${rootPath}: ${error.message}`);
        }
    }
    logger.success(`  Collected ${allActivities.length} commits`);
    return allActivities;
}
export async function saveActivitiesByDate(activities, outputDir, archiveDbDir) {
...
export async function createActivityIndex(activities, outputDir) {
...
function generateMarkdown(date, activities) {
...
//# sourceMappingURL=git-aggregator.js.map