 ...
 */
export async function loadGitActivities(dataDir, startDate, endDate) {
    const activities = [];
    try {
        // 获取日期范围内的所有日期
        ...
        logger.error(`Error loading git activities: ${error.message}`);
    }
    return activities;
}
/**
 * 从数据集获取周报数据
 */
export async function getWeeklyReportData(dataDir, weekStart) {
 ...
 * 从数据集获取月报数据
 */
export async function getMonthlyReportData(dataDir, month // YYYY-MM
) {
 ...
 * 从数据集获取简历数据（指定日期范围）
 */
export async function getResumeData(dataDir, startDate, endDate) {
    const activities = await loadGitActivities(dataDir, startDate, endDate);
    const projectMap = {};
    let totalCommits = 0;
    let totalInsertions = 0;
 ...
 * 提取项目亮点（提交信息中的关键描述）
 */
function extractHighlights(subjects) {
    const highlights = [];
    const seen = new Set();
 ...
 * 生成成就列表
 */
function generateAchievements(totalCommits, totalInsertions, projects) {
    const achievements = [];
    if (totalCommits >= 100)
        achievements.push(`累计提交 ${totalCommits} 次`);
    if (totalInsertions >= 10000)
        achievements.push(`新增代码 ${totalInsertions.toLocaleString()} 行`);
    if (projects.length >= 5)
        achievements.push(`参与 ${projects.length} 个项目开发`);
 ...
 */
export async function ensureTodayDataExists(dataDir) {
...
//# sourceMappingURL=git-data-loader.js.map